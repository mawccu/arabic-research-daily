...
/* Served as a static page (GitHub Pages) there is no Python backend.
   build_demo.py bakes the latest run into demo-data.js; when that file is
   present every call is answered locally and edits go to localStorage. */
...
function demoApi(path, opts) {
  ...
  const store = JSON.parse(localStorage.getItem('demoState') || '{}');
  ...
  if (path.startsWith('/api/run')) {
    return {
      date: D.date,
      stats: D.stats,
      shortlist: D.shortlist.map(p => {
        const saved = store[p.uid] || {};
        return {
          ...p, saved,
          verdict: saved.verdict || '',
          has_notes: Object.keys(saved).length > 0,
        };
      }),
    };
  }
  ...
  if (path.startsWith('/api/save')) {
    store[body.uid] = { ...(store[body.uid] || {}), ...body };
    localStorage.setItem('demoState', JSON.stringify(store));
    return { saved: body };
  }

  if (path.startsWith('/api/config')) return { config: D.config, ok: true };
  if (path.startsWith('/api/fulltext')) return { sections: null };

  if (path.startsWith('/api/references')) {
    const id = new URLSearchParams(path.split('?')[1] || '').get('id');
    return { references: (D.references || {})[id] || [] };
  }
...
function enterDemoMode() {
  const btn = $('#btnFetch');
  btn.disabled = true;
  btn.title = 'Fetching needs the local app';

  const bar = document.createElement('div');
  bar.className = 'demobar';
  bar.innerHTML = `<b>Static demo.</b> Real papers, real scoring — but fetching,
    full text and shared saving need the local app. Highlights and scripts you
    write here are saved to this browser only.
    <a href="https://github.com/mawccu/arabic-research-daily" target="_blank"
       rel="noopener">Run it properly →</a>`;
...
function select(p) {
  ...
  ft.disabled = DEMO || !p.pmcid;
  ft.textContent = DEMO ? 'Full text — local app only'
    : p.pmcid ? 'Load full text' : 'Abstract only (paywalled)';
